...
class InteractiveViewer(object):
    def __init__(self, img_path):
        ...
        if max(self.orig_height, self.orig_width) > max_dim:
            if self.orig_width >= self.orig_height:
                scale = max_dim / self.orig_width
                new_w = max_dim
                new_h = round(self.orig_height * scale)
            else:
                scale = max_dim / self.orig_height
                new_h = max_dim
                new_w = round(self.orig_width * scale)
    ...
    def write_tps_file(self): #new storage of annotations in tps form
        ...
        scaled_annotations = [(x * self.scale_x, y * self.scale_y) if x >= 0 else (-1, -1) for x, y in
                              self.annotations]
        ...
        lines = [f"LM={lm_count}"]
        lines += [f"{x:.3f} {y:.3f}" for x, y in valid_coords]
    ...
    def redraw_annotations(self):
        ...
        for coords in self.annotations:
            if coords is not None:
                cv2.circle(self.image, (int(round(coords[0])), int(round(coords[1]))), radius, (255, 0, 0), -1)
    ...
    def on_click(self, event):
        ...
        self.annotations[self.curr_state] = (x, y)
    ...
    def save_annotations(self):
        self.fill_default_coords()
        scaled_annotations = [(x * self.scale_x, y * self.scale_y) if x >= 0 else (-1, -1) for x, y in self.annotations]
